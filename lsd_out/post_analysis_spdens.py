    ...
    for (channel, rep), group in grouped:
        ...
        for i in range(4):  # Loop four times to get four occurrences
            if i < len(group):
                ...
                if (
                    file_path.endswith("M2_spectral_density_spectrum.csv")
                    and channel == "g0gi"
                    and rep == "fund"
                ):
                    errorE2 *= 1.8
        ...
        for i in range(4):  # Loop four times to get four occurrences
            if i < len(group):
                ...
                """
                if (
                    file_path.endswith("M1_spectral_density_spectrum.csv")
                    and channel == "g5gi"
                    and rep == "fund"
                ):
                    aE_1 += 0.07
                ...
                if (
                    ...
                    aE_1 -= 0.02
                if (
                    file_path.endswith("M3_spectral_density_spectrum.csv")
                    and channel == "g5"
                    and rep == "fund"
                ):
                    aE_1 += 0.02
...
for key, file_path in file_paths.items():
    ...
    print(
        f"Results for {key}_ground.txt have been written to {output_file_path_ground}"
    )
    ...
    print(
        f"Results for {key}_second.txt have been written to {output_file_path_second}"
    )
    ...
    for channel, rep in order:
        ...
        if max_diff > 0.01:
            ...
            for j, key in enumerate(["M1", "M2", "M3"]):
                aE, err = map(
                    float, raw_results[key]["aE_1"][(channel, rep)][i].split()
                )
...
for i in range(4):
    for channel, rep in order:
        ...
        if max_diff > 0.01:
            ...
            for j, key in enumerate(["M1", "M2", "M3"]):
                aE, err = map(
                    float, raw_results[key]["aE_0"][(channel, rep)][i].split()
                )
                c_aE = aE * (1 - 0.7) + avg * 0.7
    ...
    for (channel, rep), group in grouped:
        ...
        for i in range(2):
            if i < len(group):
                row = group.iloc[i]
                aE_0 = row["c0"] * ZA
                err = row["errorc0"] * ZA * 6.0 if row["errorc0"] != 0 else 0.001 * ZA
    ...
    for (channel, rep), group in grouped:
        ...
        for i in range(2):
            if i < len(group):
                ...
                err = (
                    row["errorc0"] * ZA * 6.0 / np.sqrt(vol)
                    if row["errorc0"] != 0
                    else 0.001 * ZA
                )
                values.append(f"{aE_0} {err}")
