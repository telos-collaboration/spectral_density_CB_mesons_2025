...
def perform_fit(
    ...
    if fit_peaks_switch == 0:
        print(
            LogMessage(),
            f"Ens: {ensemble}, Repr: {rep}, Channel: {channel}, Kernel: {kernel}, No. Peaks: {old_k_peaks}",
        )
    ...
    file_names = sorted(
        file_names, key=lambda x: float(x.split("E")[1].split("sig")[0])
    )
    # print(file_names)
    ...
    if fit_peaks_switch == 0:
        ...
        params.add(
            "amplitude_1",
            value=amp1_fit,
            min=amp1_fit - 0.4 * amp1_fit,
            max=amp1_fit + 0.4 * amp1_fit,
        )
        params.add("mean_1", value=1.0, min=0.99, max=1.01)
        params.add(
            "amplitude_2",
            value=amp2_fit,
            min=amp2_fit - 0.4 * amp2_fit,
            max=amp2_fit + 0.4 * amp2_fit,
        )
        params.add(
            "mean_2", value=mean2_fit, min=mean2_fit - 0.02, max=mean2_fit + 0.02
        )
        if triple_fit is True:
            params.add(
                "amplitude_3",
                value=amp3_fit,
                min=amp3_fit - 0.4 * amp3_fit,
                max=amp3_fit + 0.4 * amp3_fit,
            )
            params.add(
                "mean_3", value=mean3_fit, min=mean3_fit - 0.15, max=mean3_fit + 0.15
            )
        if four_fit is True:
            params.add(
                "amplitude_4",
                value=amp4_fit,
                min=amp4_fit - 0.4 * amp4_fit,
                max=amp4_fit + 0.4 * amp4_fit,
            )
            params.add(
                "mean_4", value=mean4_fit, min=mean4_fit - 0.4, max=mean4_fit + 0.4
            )
    elif fit_peaks_switch == 1:
        params = Parameters()
        params.add(
    ...
    for k in range(nboot):
        ...
        if fit_peaks_switch == 0:
            print(
                LogMessage(),
                f"Ens: {ensemble}, Repr: {rep}, Channel: {channel}, Kernel: {kernel}, No. Peaks: {old_k_peaks}, Bootstrap Fit number:",
                k,
                f"/ {nboot} done.",
            )
        elif fit_peaks_switch == 1:
    ...
    with open(
        f"../CSVs/{ensemble}_spectral_density_spectrum.csv", "a", newline=""
    ) as csvfile:
        csvwriter = csv.writer(csvfile)
        formatted_values = [f"{float(val):.4f}" for val in values]
        if fit_peaks_switch == 0:
            csvwriter.writerow(
                [ensemble, kernel, rep, channel, k_peaks] + formatted_values
            )
        elif fit_peaks_switch == 1:
            csvwriter.writerow(
                [ensemble, kernel, rep, channel, k_peaks + 1] + formatted_values[:-2]
            )
            ...
            for kernel in kerneltype:
                for fit_peaks_switch in range(2):
                    ...
                    perform_fit(
                        ...
                        flag_chi2,
                        matrix_4D,
                        k_peaks[ensemble][channel_num],
                        kernel,
                        Nboot_fit[ensemble_num],
                        fit_peaks_switch,
                        matrix_2D,
                    )
