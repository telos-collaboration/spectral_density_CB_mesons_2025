...
for n in range(3):
    for index, ensemble in enumerate(ensembles):
        ...
        for chunk in pd.read_csv(f'./CSVs/{ensemble}_chimerabaryons_spectral_density_spectrum.csv', chunksize=chunk_size):
            ...
            print(channel)
            print(repr)
            if channel == 'Chimera_OC_even' and repr == 'fund':
                CHANNEL = 'PS'
                try:
                    ...
                    channel_E0 = '-'
                    err_channel_E0 = '-'
            elif channel == 'Chimera_OC_odd' and repr == 'fund':
                CHANNEL = 'V'
                try:
                    row2 = gevp[gevp['ENS'].str.contains(prefix[index])]
                    channel_E0 = round(row2[f"{channel}_E{n}"].values[0], 4)
                    ...
                    channel_E0 = '-'
                    err_channel_E0 = '-'
            elif channel == 'Chimera_OV12_even' and repr == 'fund':
                CHANNEL = 'T'
                try:
                    row2 = gevp[gevp['ENS'].str.contains(prefix[index])]
                    channel_E0 = round(row2[f"{channel}_E{n}"].values[0], 4)
            ...
            elif channel == 'Chimera_OV12_odd' and repr == 'fund':
                CHANNEL = 'AV'
                try:
                    row2 = gevp[gevp['ENS'].str.contains(prefix[index])]
                    channel_E0 = round(row2[f"{channel}_E{n}"].values[0], 4)
                    ...
                    err_channel_E0 = '-'
                # Add similar try-except blocks for other conditions
            elif channel == 'Chimera_OV32_even' and repr == 'fund':
                CHANNEL = 'AT'
                try:
                    row2 = gevp[gevp['ENS'].str.contains(prefix[index])]
                    channel_E0 = round(row2[f"{channel}_E{n}"].values[0], 4)
                    ...
                    err_channel_E0 = '-'
                # Add similar try-except blocks for other conditions
            elif channel == 'Chimera_OV32_odd' and repr == 'fund':
                CHANNEL = 'S'
                try:
                    row2 = gevp[gevp['ENS'].str.contains(prefix[index])]
                    channel_E0 = round(row2[f"{channel}_E{n}"].values[0], 4)
