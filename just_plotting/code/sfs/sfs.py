...
def fit_spectral_density_1_single(energy, spectral_density1):
    def fit_single_bootstrap(spectral_density1_sample, cholesky_cov):
        ...
        minimizer = Minimizer(chisq_correlated, params, fcn_args=(energy, spectral_density1_sample, cholesky_cov))
        #print(cholesky_cov)
...
def chisq_correlated_2(params, energy, spectral_density_sample, cholesky_cov, fixed_params):
    a0, E0 = fixed_params
...
def fit_spectral_density_2_single_fixed_params(energy, spectral_density2, fixed_params, mpi):
    def fit_single_bootstrap(spectral_density2_sample, cholesky_cov):
        params = Parameters()
        params.add('c0', value=0.01)

        minimizer = Minimizer(chisq_correlated_2, params, fcn_args=(energy, spectral_density2_sample, cholesky_cov, fixed_params))
        result = minimizer.minimize()
...
def plot_with_errors_single(energy, avg_spectral_density1, avg_spectral_density2, fit_params_1, fit_params_2, num_bootstraps, spectral_density, mpi):
    ...
    fit_curves_1 = np.array([spectral_density_1_single(energy_fine, *params) for params in fit_params_1])
    ...
    fit_curves_2 = np.array([spectral_density_2_single(energy_fine, c0, E0, fit_params_1[0][0]) for c0 in fit_params_2])
    ...
    plt.fill_between(energy_fine, mean_fit_curve_1 - 2.2*std_fit_curve_1, mean_fit_curve_1 + 2.2*std_fit_curve_1, color=CB_color_cycle[1], alpha=0.15)
    ...
    plt.fill_between(energy_fine, mean_fit_curve_2 - 1.8*std_fit_curve_2, mean_fit_curve_2 + 1.8*std_fit_curve_2, color=CB_color_cycle[2], alpha=0.15)
...
def main():
    ...
    fit_params_1_mean, fit_params_1_std, fit_params_1 = fit_spectral_density_1_single(energy, spectral_density[:, 0, :])
    ...
    fit_params_2_mean, fit_params_2_std, fit_params_2 = fit_spectral_density_2_single_fixed_params(energy, spectral_density[:, 1, :], fit_params_1_mean, mpi)
    ...
    '''
    # Plot the results
    plot_with_errors_single(energy, avg_spectral_density1, avg_spectral_density2, fit_params_1, fit_params_2, spectral_density.shape[0], spectral_density, mpi)
