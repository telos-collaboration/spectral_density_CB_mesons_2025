...
rule w0:
    ...
    input:
        ...
        script="plateaus/flow.py",
    output:
        mean=f"JSONs/{dir_template}/w0_mean.csv",
        samples=f"JSONs/{dir_template}/w0_samples.json",
    conda:
...
rule w0_flow_plot:
    ...
    input:
        data=f"raw_data/flows/{dir_template}/out_wflow",
        script="plateaus/plot_w_flow.py",
    output:
...
rule topological_charge:
    ...
    input:
        data=f"raw_data/flows/{dir_template}/out_wflow",
        script="plateaus/top_charge.py",
    output:
        data=f"JSONs/{dir_template}/top_charge.csv",
    conda:
        "../envs/flow_analysis.yml"
    shell:
...
def all_flow_data(wildcards):
    return [
        f"JSONs/{dir_template}/{basename}.csv".format(**row)
...
rule gflow_table:
    ...
    input:
        data=all_flow_data,
        script="plateaus/tables/Q_table.py",
    output:
