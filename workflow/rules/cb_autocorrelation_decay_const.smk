...
rule instantiate_julia:
    ...
    shell:
        f"cd {cb_julia_source}/.. && julia ../{{input.script}} && cd .. && touch {{output.marker}}"
...
rule analyse_flows:
    ...
    conda:
        "../envs/CB_autocorrelation_decay_constant.yml"
    shell:
        f"julia --project={cb_julia_source} {cb_julia_source}/autocorrelation.jl --wilson_flow_hdf5 {{input.flows}} --wall_correlators_hdf5 {{input.correlators}} --output_hdf5 {{output.h5}} --output_tex {{output.table}} --plot_dir extra_assets/autocorrelation_plots"


rule mass_wall:
    params:
        plateau_start=lambda wildcards: lookup(
            within=metadata,
            query="ensemble_name == '{wildcards.ensemble_name}'",
            cols=f"wall_{wildcards.representation}_{wildcards.channel}_plateau_start",
        ),
        plateau_end=lambda wildcards: lookup(
            within=metadata,
            query="ensemble_name == '{wildcards.ensemble_name}'",
            cols=f"wall_{wildcards.representation}_{wildcards.channel}_plateau_end",
        ),
    input:
        correlators="data_assets/wall_correlators.h5",
    ...
    conda:
        "../envs/CB_autocorrelation_decay_constant.yml"
    shell:
        "python {input.script} --ensemble_name {wildcards.ensemble_name}/{wildcards.representation} --plateau_start {params.plateau_start} --plateau_end {params.plateau_end} --output_file_mean {output.csv} --channel {wildcards.channel} {input.correlators}"
